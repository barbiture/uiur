        ...
        <>
            {navArray.map(([title, url, id]) => (
                <Link
                    href={url}
                    key={id}
                    className={`${currentRoute === url ? 'text-brand-yellow' : 'text-white'} text-center basis-1/4 self-center mx-10 my-5 font-medium hover:text-brand-yellow hover:text-slate-900`}>
                    {title}
                </Link>
            ))}
