...
import {ChildProps, IFormValues, InputProps, TitleProps, IStylesForm} from './form.d';
...
const stylesForm: IStylesForm = {
    team: {
        ...
        radio: 'rounded-lg block p-5 bg-brand-yellow w-full text-center peer-checked:text-brand-blue peer-checked:bg-white cursor-pointer',
        formWrapper: ''
    },
    home: {
        ...
        radio: 'block p-5 bg-brand-yellow w-full text-center peer-checked:text-white peer-checked:bg-brand-blue cursor-pointer',
...
const Radio = ({name, register, typePage, value, type}: InputProps) => {
    return <div className="w-full">
        <input {...register} id={value} type={type} name={register.name}
               value={value} className="peer hidden"/>
        <label htmlFor={value}
               className={styleSwitch.get(typePage)?.radio} >{name}</label>
    </div>
}
...
const Input = ({name, register, typePage, widthMaskPhone}: InputProps) => {
    return (
        <InputWrapper>
            ...
            {widthMaskPhone ? <InputMask  {...register}
...
const TextArea = ({name, register, typePage}: InputProps) => {
    return (
        <InputWrapper>
            ...
            <textarea
                {...register}
                className={styleSwitch.get(typePage)?.input}
            />
        </InputWrapper>
    );
};
...
const FormTitle = ({typePage}: TitleProps) => {
    const intl = useIntl();
    return <h2 className={`${styleSwitch.get(typePage)?.text} text-center mb-6`}>
...
const Form = ({typePage}: TitleProps) => {
    ...
    return (
        <div className={`${styleSwitch.get(typePage)?.form || ''} w-full p-6 font-eUkraine sm:text-base text-xs`}>
            <div className={styleSwitch.get(typePage)?.formWrapper}>
                <FormTitle typePage={typePage}/>
                <div className={`pb-4 text-error font-light ${!Object.values(errors).length && 'invisible'} `}>
                ...
                >
                    <div className="grid grid-rows-3 gap-5">
                        ...
                        <div className="grid grid-cols-2 gap-5">
                            <Items.Radio typePage={typePage} name={intl.formatMessage({id: 'form.radio.participant'})}
                                         type="radio"
                                         register={{...register('LeadType', {required: true})}}
                                         value="participant"/>
                            <Items.Radio typePage={typePage} name={intl.formatMessage({id: 'form.radio.volunteer'})}
                                         type="radio"
                                         value="volunteer" register={{...register('LeadType', {required: true})}}
                                         required/>
                        </div>
                        <input
                            value={intl.formatMessage({id: 'form.join'})}
                            type="submit"
                            className={`p-5 bg-brand-yellow row-span-2 border-0 font-thin text-brand-blue cursor-pointer ${styleSwitch.get(typePage)?.button}  ${styleSwitch.get(typePage)?.input}`}
                        />
                    </div>
                </form>
            </div>
